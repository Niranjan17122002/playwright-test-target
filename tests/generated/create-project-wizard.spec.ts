...
test('POSITIVE: Select the Development environment option in the wizard', async ({ page }) => {
  test.setTimeout(120000);
  ...
  const projectName = `Dev Project ${Date.now()}`;
  test.info().annotations.push({ type: 'test-data', description: `projectName: ${projectName}` });
  ...
  await page.locator('#wizard-environment').selectOption('development');
  await page.locator('#wizard-description').fill('End-to-end wizard test project using Development environment');
  await page.locator('#wizard-next-2').click();
  await expect(page.locator('#wizard-step-3')).toBeVisible();
  await expect(page.locator('#wizard-progress')).toHaveText('Step 3 of 3');

  await expect(page.locator('#wizard-review-name')).toHaveText(projectName);
  await expect(page.locator('#wizard-review-environment')).toHaveText('development');
  await expect(page.locator('#wizard-review-description')).toHaveText('End-to-end wizard test project using Development environment');
